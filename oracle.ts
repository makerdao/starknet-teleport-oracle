...
export async function generateAttestation(event: Event): Promise<{ signatures: string; wormholeGUID: WormholeGUID }> {
  ...
  const message = ethers.utils.defaultAbiCoder.encode(["bytes32", "bytes32", "bytes32", "bytes32", "uint128", "uint80", "uint48"], event.data]);
